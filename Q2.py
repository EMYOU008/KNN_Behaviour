...
def sample(size):
    ...
    plt.ylabel("Time")
    plt.legend(("sklearn Classifier","My Classifier"))
...
def sample_1(dimension):
    ...
    for d in range(1,dimension):
        ...
        (x_train,y_train)=getExamples(200,d)  #taking random take for training from getexample function from NN_Plotit file
        start=time()   #time starting when training start
        kitkat.fit(x_train,y_train)  #fit the model foe the given input


        (x_test,y_test)=getExamples(200,d) # taking data for the test purpose
        y_predict=kitkat.predict(x_test) #predict the labels against the features
...
def sample_2(cluster):
    ...
    for k in range(1,cluster,2):
        ...
        u = mars.predict(x_test)

        Run_time.append(accuracy(y_test, u))
    #print("my .............." + str(Run_time))
...
def sample_3(cluster):
    ...
    for k in range(1,cluster,2):
        ...
        u=mars.predict(x_train)

        Run_time.append(accuracy(y_train, u))
    #print("my .............." + str(Run_time))
